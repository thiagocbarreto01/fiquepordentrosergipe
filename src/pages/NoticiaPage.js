...
import { getNoticiaBySlug, getMostReadNoticias, subscribeToNoticiasFeed, timeAgo } from "@/lib/noticias";
...
export default function NoticiaPage() {
    ...
    const [contentLightbox, setContentLightbox] = useState(null);
    useEffect(() => {
        if (!post)
            return;
        ...
        imgs.forEach((img) => {
            ...
            if (!img.getAttribute("title"))
                img.setAttribute("title", post.title);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const seoDesc = p.ai_summary || p.meta_description || p.subtitle || p.title;
    ...
    const aiTitle = post.ai_seo_title || post.meta_title || post.title;
    const aiSummary = post.ai_summary || post.meta_description || post.subtitle || post.excerpt || post.title;
    ...
    return (<SiteLayout>
      ...
      <article className="container-news mt-4 grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2">
          <div className="flex items-center gap-2 mb-3">
            ...
            {post.categories?.name && (<Link to={`/categoria/${post.categories.slug}`} className="category-tag hover:underline">
                {post.categories.name}
              </Link>)}
            {isStaff && (<Button size="sm" variant="outline" className="ml-auto gap-2" onClick={() => setReelOpen(true)}>
                <Film className="h-4 w-4"/> Gerar Reel
              </Button>)}
          </div>
          {isStaff && (<ReelGeneratorDialog open={reelOpen} onOpenChange={setReelOpen} post={post}/>)}
          ...
          {post.subtitle && (<p className="mt-4 text-lg md:text-xl text-muted-foreground font-serif-news">{post.subtitle}</p>)}
        ...
        try {
            ...
            window.location.href = url;
    ...
    } }} className="p-2 hover:bg-secondary rounded-sm"><MessageCircle className="h-4 w-4"/></a>
              ...
              <button aria-label="Compartilhar link com prévia" title="Compartilhar link com prévia" onClick={async () => {
            ...
            toast("Link com prévia copiado");
              ...
              <button aria-label="Copiar link com prévia" title="Copiar link com prévia" onClick={async () => {
            const shareUrl = getSocialShareUrl(post.slug);
            await navigator.clipboard.writeText(shareUrl);
            toast("Link com prévia copiado");
        }} className="p-2 hover:bg-secondary rounded-sm">
              ...
              <button aria-label="Link do site" title="Link do site" onClick={async () => {
          ...
          {(() => {
            ...
            const providerLabel = mainVideo?.provider === "youtube" ? "YouTube" :
            ...
            return (<>
                ...
                {mainVideo ? (<figure className="mt-6">
                    <VideoEmbed url={post.video_url_principal} title={post.title}/>
                    <figcaption className="text-xs text-muted-foreground mt-2">
                      Vídeo: {providerLabel}
                    </figcaption>
                  </figure>) : mainImageUrl ? (<figure className="mt-6 mx-auto max-w-[760px]">
                    <button type="button" onClick={() => setLightboxOpen(true)} className="group block w-full cursor-zoom-in overflow-hidden rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" aria-label="Ampliar imagem">
                      <AdaptiveCoverImage src={mainImageUrl} alt={imgAlt} loading="eager" fetchPriority="high" onError={(e) => handleImgError(e, post)}/>
                    </button>
                    {(caption || credit) && (<figcaption className="mt-2 text-sm text-muted-foreground leading-snug">
                        {caption && <span>{caption}</span>}
                        {caption && credit && <span className="mx-1">·</span>}
                        {credit && <span className="italic">Crédito: {credit}</span>}
                      </figcaption>)}
                    <ImageLightbox src={mainImageUrl} alt={imgAlt} caption={caption} credit={credit} open={lightboxOpen} onClose={() => setLightboxOpen(false)}/>
                  </figure>) : null}

                <div className="article-content prose prose-lg max-w-none mt-6 font-serif-news leading-relaxed text-foreground/90">
                  {looksHtml ? (<div dangerouslySetInnerHTML={{
                ...
                {relatedVideos.length > 0 && (<section className="mt-8 border-t border-border pt-6">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {relatedVideos.map((v, i) => (<VideoEmbed key={`${v.url}-${i}`} url={v.url} title={`Vídeo ${i + 1}`}/>))}
                    </div>
                  </section>)}
              </>);
        })()}
          ...
          {related.length > 0 && (<section className="mt-8 border-t border-border pt-6">
              <h2 className="font-display font-black text-xl mb-4 uppercase tracking-tight">
                Mais sobre este assunto
              </h2>
              <ul className="space-y-3">
                {related.map((r) => (<li key={r.id}>
                    <Link to={`/noticia/${r.slug}`} className="group flex gap-3 items-start hover:bg-secondary/30 -mx-2 px-2 py-2 rounded-sm">
                      {r.cover_image_url && (<img src={r.cover_image_url} alt="" className="w-20 h-16 object-cover rounded-sm shrink-0" loading="lazy"/>)}
                      <div>
          ...
          {post.tags && post.tags.length > 0 && (<div className="mt-6 flex flex-wrap gap-2">
              {post.tags.map((t) => (<span key={t} className="px-2 py-1 bg-secondary text-xs uppercase tracking-wider font-bold">#{t}</span>))}
            </div>)}
      ...
      {contentLightbox && (<ImageLightbox src={contentLightbox.src} alt={contentLightbox.alt} open={!!contentLightbox} onClose={() => setContentLightbox(null)}/>)}
    </SiteLayout>);
}
