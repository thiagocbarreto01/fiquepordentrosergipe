...
import { AlertTriangle, ExternalLink, Sparkles, RotateCcw, Loader2, CheckCircle2, BrainCircuit, ThumbsUp, ThumbsDown, Pin, PinOff, Film, Share2 } from "lucide-react";
...
import { STATUS_LABEL, STATUS_COLOR, MATCH_REASON_LABEL, normalizeStatus, } from "@/lib/statusFlow";
...
import { deriveEditorRole, getPrimaryAction } from "@/lib/editorActions";
...
export default function AdminPostEditor() {
    ...
    const [form, setForm] = useState({
        ...
        ai_rewrite_quality: "jornalistica",
        ai_review_status: "pendente",
        meta_keywords: [],
        video_url_principal: "",
        ...
        supabase
            ...
            setForm({
                ...
                scheduled_at: data.scheduled_at
                    ...
                    : "",
                home_expires_at: data.home_expires_at
                    ? new Date(data.home_expires_at).toISOString().slice(0, 16)
                    : "",
                main_featured_expires_at: data.main_featured_expires_at
                    ? new Date(data.main_featured_expires_at).toISOString().slice(0, 16)
                    : "",
                is_evergreen: !!data.is_evergreen,
                is_main_featured: !!data.is_main_featured,
                video_url_principal: data.video_url_principal ?? "",
                videos_relacionados_text: (data.videos_relacionados ?? []).join("\n"),
            });
            const serverStamp = data.updated_at ?? data.created_at;
            if (serverStamp)
                setLastServerSavedAt(new Date(serverStamp));
    ...
    const { savedAt: localSavedAt, readDraft, clearDraft, key: autosaveKey, } = useEditorAutosave({
        userId: user?.id,
        postId: isNew ? "new" : id,
        data: form,
        enabled: hydrated,
    ...
    const [localSavedAtDisplay, setLocalSavedAtDisplay] = useState(null);
    ...
    useEffect(() => {
        if (!hydrated || !user?.id || recoverCheckedRef.current)
            return;
    ...
    useEffect(() => {
        if (saveState.kind === "saving" || saveState.kind === "saved" || saveState.kind === "error")
            return;
        if (dirty && localSavedAtDisplay)
            setSaveState({ kind: "local", at: localSavedAtDisplay });
        else if (dirty)
    ...
    async function uploadCover(file) {
        ...
        try {
            ...
            setForm((f) => ({ ...f, manual_image_url: data.publicUrl }));
            if (hydrated)
                setDirty(true);
    ...
    async function save(targetStatus) {
        ...
        const restricted = ["aprovada", "publicada", "rejeitada"];
        ...
        const videosRelacionados = (form.videos_relacionados_text ?? "")
            .split(/\r?\n/)
            .map((s) => s.trim())
            .filter(Boolean);
        ...
        const payload = {
            ...
            published_at: finalStatus === "publicada"
        ...
        let res;
        if (isNew)
            res = await supabase.from("posts").insert(payload).select("id").maybeSingle();
        else
            res = await supabase.from("posts").update(payload).eq("id", id).select("id").maybeSingle();
        setSaving(false);
        if (res.error) {
    ...
    async function handleSchedulePublish(isoUtc) {
        if (!canPublish)
            return toast.error("Apenas editor/admin pode agendar");
        if (isNew || !id) {
            toast.error("Salve a notícia antes de agendar");
            return;
        }
        if (dirty) {
            toast.error("Salve as alterações antes de agendar");
            return;
        }
        setSaving(true);
        ...
        const scheduledAt = data?.scheduled_at ?? isoUtc;
        ...
        setForm((f) => ({
            ...
            scheduled_from_status: data?.scheduled_from_status ?? f.scheduled_from_status ?? null,
        }));
        setDirty(false);
    ...
    async function handleCancelSchedule() {
        if (!canPublish)
            return toast.error("Apenas editor/admin pode cancelar agendamento");
        if (isNew || !id)
            return;
        ...
        const newStatus = (data?.status ?? form.status);
        setForm((f) => ({
            ...f,
            status: newStatus,
            scheduled_at: "",
    ...
    async function gerarComIA() {
        ...
        try {
            ...
            if (error) {
                const msg = error.context?.error || error.message || "Falha na IA";
    ...
    async function marcarRevisadoPorEditor() {
        ...
        const { error } = await supabase
            .from("posts")
            .update({ ai_review_status: "revisado_editor" })
            .eq("id", id);
        if (error)
            return toast.error(error.message);
        setForm((f) => ({ ...f, ai_review_status: "revisado_editor" }));
    ...
    async function analisarRelevancia() {
        if (isNew)
            return toast.error("Salve a notícia antes de analisar");
    ...
    function flagsForPlacement(placement) {
        switch (placement) {
    ...
    async function aceitarSugestaoIA() {
        ...
        if (error)
            return toast.error(error.message);
        await supabase.from("home_audit").insert({
        ...
        setForm((f) => ({ ...f, ...flags, ai_suggestion_status: "aceita" }));
    ...
    async function ignorarSugestaoIA() {
        ...
        setForm((f) => ({ ...f, ai_suggestion_status: "ignorada" }));
    ...
    async function marcarDestaqueManual() {
        ...
        setForm((f) => ({ ...f, is_featured: true, ai_suggestion_status: "ignorada" }));
    ...
    async function removerDestaque() {
        ...
        setForm((f) => ({ ...f, is_featured: false, is_main_featured: false, is_urgent: false }));
        toast.success("Destaque removido");
    }
    async function aplicarVersao(versao) {
    ...
    const handleEditorAction = (kind) => {
        switch (kind) {
            ...
            case "open_public": {
                ...
                if (s)
                    window.open(`/noticia/${s}`, "_blank", "noopener,noreferrer");
                return;
            }
        }
    };
    return (<AdminLayout>
      ...
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur-sm border-b border-border -mx-4 px-4 py-3 mb-6 flex items-center justify-between gap-4 shadow-md">
        <div className="flex items-center gap-3">
          ...
          {!isNew && (<span className={`px-3 py-1 text-[10px] font-bold uppercase border rounded-full shadow-sm ${STATUS_COLOR[currentStatus]}`}>
              {STATUS_LABEL[currentStatus]}
            </span>)}
        </div>

        <div className="flex items-center gap-2">
          {!isNew && (<Button onClick={() => setReelOpen(true)} variant="outline" size="sm" className="gap-2" title="Gerar Reel vertical 1080x1920 a partir desta notícia">
      ...
      {(() => {
            ...
            const map = {
                ...
                local: { cls: "bg-blue-50 border-blue-200 text-blue-900", label: <><strong>Rascunho local salvo</strong> às {st.at?.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}. Somente neste navegador — o servidor ainda não recebeu.</> },
                ...
                saved: { cls: "bg-emerald-50 border-emerald-200 text-emerald-900", label: <><CheckCircle2 className="inline h-3.5 w-3.5 mr-1"/> <strong>Salvo no servidor</strong> às {st.at?.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}.</> },
                error: { cls: "bg-red-50 border-red-200 text-red-900", label: <><AlertTriangle className="inline h-3.5 w-3.5 mr-1"/> <strong>Erro ao salvar:</strong> {st.message}</> },
            };
            const info = map[st.kind];
            if (!info)
                return null;
            return (<div className={`mb-4 -mt-2 flex items-center justify-between gap-3 flex-wrap text-xs px-3 py-2 rounded-sm border ${info.cls}`}>
            <span>{info.label}</span>
            {(st.kind === "dirty" || st.kind === "local") && localSavedAtDisplay && (<button type="button" onClick={() => { clearDraft(); setLocalSavedAtDisplay(null); }} className="underline font-bold">
      ...
      <PublishDialog open={publishOpen} onOpenChange={setPublishOpen} submitting={saving} canOverrideCover={canPublish} form={{
            title: form.title,
            ...
            pinned_until: null,
            pinned_reason: null,
        }} onConfirm={async () => { await save("publicada"); setPublishOpen(false); }} canSchedule={canPublish} onSchedule={async (iso) => { await handleSchedulePublish(iso); }}/>

      {/* ArticlePreviewDialog — desktop/mobile */}
      <ArticlePreviewDialog open={previewOpen} onOpenChange={setPreviewOpen} snapshot={{
            title: form.title,
            subtitle: form.subtitle,
            content: form.content,
            cover_image_url: form.cover_image_url,
      ...
      <UnsavedChangesDialog open={guard.hasPending} onContinueEditing={guard.cancel} onDiscard={guard.confirmDiscard}/>

      {/* Recuperação de rascunho local (nunca aplica sozinho) */}
      <RecoverDraftDialog open={recoverOpen} savedAt={recoveredPayload?.savedAt ?? null} onRecover={() => {
      ...
      {!isNew && (<ReelGeneratorDialog open={reelOpen} onOpenChange={setReelOpen} post={{ ...form, id }}/>)}
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3 hidden">
        ...
        {!isNew && (<span className={`px-3 py-1 text-xs font-bold uppercase border ${STATUS_COLOR[currentStatus]}`}>
            Status atual: {STATUS_LABEL[currentStatus]}
          </span>)}
      </div>

      {!isNew && form.slug && (<div className="mb-4 flex flex-wrap items-center gap-2">
          <Button type="button" size="sm" className="gap-2" onClick={async () => {
          ...
          <Button type="button" size="sm" variant="outline" className="gap-2" onClick={async () => {
      ...
      {!isNew && (() => {
            ...
            return (<div className="mb-6 bg-card border border-border p-4 rounded-sm">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                {form.ai_version_used && form.ai_version_used !== "original" && (<div className="text-[10px] text-muted-foreground mt-0.5">
                    Versão usada: <strong>{form.ai_version_used}</strong>
                  </div>)}
              </div>
              {form.source_url && (<div className="sm:col-span-2 md:col-span-4">
                  <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">URL original</div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <a href={form.source_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline break-all">
                      {form.source_url}
                    </a>
      ...
      {(duplicateOriginal || duplicateMatches.length > 0) && (<div className="mb-6 border-2 border-orange-400 bg-orange-50 p-4 rounded-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 space-y-3">
              ...
              {duplicateOriginal && (<div className="bg-white border border-orange-300 p-3 rounded-sm">
                  ...
                  <div className="flex gap-3 mt-2 text-sm">
                    <Link to={`/admin/posts/${duplicateOriginal.id}`} className="text-orange-700 underline font-bold inline-flex items-center gap-1">
                      Abrir original <ExternalLink className="h-3 w-3"/>
                    </Link>
                    {normalizeStatus(duplicateOriginal.status) === "publicada" && (<a href={`/noticia/${duplicateOriginal.slug}`} target="_blank" rel="noreferrer" className="text-orange-700 underline inline-flex items-center gap-1">
                        Ver no site <ExternalLink className="h-3 w-3"/>
                      </a>)}
                  </div>
                </div>)}

              {duplicateMatches.length > 0 && (<div>
                  <div className="text-xs uppercase font-bold text-orange-700 tracking-wider mb-1">
                    Outras semelhantes
                  </div>
                  <ul className="space-y-1 text-sm">
                    {duplicateMatches.slice(0, 5).map((m) => (<li key={m.id} className="bg-white border border-orange-200 px-3 py-2 rounded-sm">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <Link to={`/admin/posts/${m.id}`} className="text-xs text-orange-700 underline whitespace-nowrap">
                            comparar
                          </Link>
                        </div>
                      </li>))}
      ...
      {!isNew && (<div className="mb-6 border border-border bg-card p-4">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {form.ai_suggestion_status === "aceita" && (<span className="text-xs px-2 py-0.5 bg-emerald-500/15 text-emerald-700 border border-emerald-500/40 uppercase font-bold">Sugestão aceita</span>)}
              {form.ai_suggestion_status === "ignorada" && (<span className="text-xs px-2 py-0.5 bg-muted text-muted-foreground border border-border uppercase font-bold">Sugestão ignorada</span>)}
            </div>
          ...
          {form.relevance_score != null ? (<div className="space-y-3">
              {form.relevance_reason && (<div className="text-sm">
                  <span className="text-xs uppercase font-bold tracking-wider text-muted-foreground mr-2">Motivo:</span>
                  {form.relevance_reason}
                </div>)}
              {form.ai_suggested_placement && (<div className="border border-dashed border-primary/40 bg-primary/5 p-3 flex items-center justify-between flex-wrap gap-2">
              ...
              {form.relevance_factors && typeof form.relevance_factors === "object" && (<details className="text-xs">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-1 mt-2">
                    {Object.entries(form.relevance_factors).map(([k, v]) => (<div key={k} className="flex justify-between border-b border-border py-0.5">
                        <span className="capitalize">{k.replace(/_/g, " ")}</span>
                        <span className="font-mono font-bold">{String(v)}</span>
                      </div>))}
                  </div>
                </details>)}
              {form.relevance_analyzed_at && (<div className="text-xs text-muted-foreground">Analisado em {new Date(form.relevance_analyzed_at).toLocaleString("pt-BR")}</div>)}
      ...
      {!isNew && (<div className="mb-6 border border-border bg-card p-4">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {form.ai_rewritten_at && (<span className="text-xs text-muted-foreground">
                  · gerada em {new Date(form.ai_rewritten_at).toLocaleString("pt-BR")}
                </span>)}
              {form.ai_version_used && (<span className="text-xs px-2 py-0.5 border border-primary text-primary uppercase font-bold">
                  Em uso: {form.ai_version_used === "gerada" ? "IA" : "Original"}
                </span>)}
              {form.ai_review_status === "reescrito_ia" && (<span className="text-xs px-2 py-0.5 bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/40 uppercase font-bold">
                  Reescrito pela IA
                </span>)}
              {form.ai_review_status === "revisado_editor" && (<span className="text-xs px-2 py-0.5 bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/40 uppercase font-bold">
                  Revisado por editor
                </span>)}
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <select value={form.ai_rewrite_quality || "jornalistica"} onChange={(e) => setForm((f) => ({ ...f, ai_rewrite_quality: e.target.value }))} className="text-xs border border-border bg-background px-2 py-1.5 font-bold uppercase tracking-wider" title="Nível de qualidade da reescrita">
                <option value="basica">Básica</option>
                <option value="jornalistica">Jornalística (padrão)</option>
                <option value="premium">Premium</option>
              </select>
              <Button type="button" size="sm" onClick={gerarComIA} disabled={generatingAI} className="bg-primary text-primary-foreground">
                {generatingAI ? (<><Loader2 className="h-4 w-4 mr-1 animate-spin"/> Gerando…</>) : (<><Sparkles className="h-4 w-4 mr-1"/> {form.titulo_gerado ? "Regerar com IA" : "Gerar com IA"}</>)}
              </Button>
              {!isNew && form.ai_review_status === "reescrito_ia" && (<Button type="button" size="sm" variant="outline" onClick={marcarRevisadoPorEditor}>
                  Marcar revisado
                </Button>)}
            </div>
          </div>

          {(form.titulo_original || form.titulo_gerado) ? (<div className="grid md:grid-cols-2 gap-3">
              <div className="border border-border p-3 bg-background">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => aplicarVersao("original")} disabled={!form.titulo_original && !form.conteudo_original}>
              ...
              <div className="border border-primary p-3 bg-primary/5">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button type="button" size="sm" onClick={() => aplicarVersao("gerada")} disabled={!form.titulo_gerado} className="bg-primary text-primary-foreground">
                ...
                {form.resumo_gerado && (<div className="text-xs italic text-muted-foreground mb-1">{form.resumo_gerado}</div>)}
                <div className="text-xs text-muted-foreground line-clamp-6 whitespace-pre-wrap">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <EditorPrincipalSection values={{
            title: form.title ?? "",
            subtitle: form.subtitle ?? "",
            category_id: form.category_id ?? "",
            tags: form.tags ?? "",
        }} categories={cats} onChange={(patch) => {
            setForm((f) => {
                const next = { ...f, ...patch };
        ...
        }} categoryName={cats.find((c) => c.id === form.category_id)?.name || ""} sourceName={sourceName || ""} uploading={uploading} isNew={isNew} onChange={(patch) => setForm((f) => ({ ...f, ...patch }))} onUpload={uploadCover} onReprocess={reprocessImage}/>



          <EditorContentSection values={{ content: form.content ?? "" }} textareaRef={contentRef} onChange={(patch) => setForm((f) => ({ ...f, ...patch }))} aiActions={<>
                {!isNew && isStaff && (<CompletePostAIDialog postId={id} currentTitle={form.title || ""} currentContent={form.content || ""} onApply={(r) => setForm({
                    ...form,
                ...
                {!isNew && isAdmin && siteSettings?.recapture_assisted_enabled && id && (<RecaptureDialog postId={id} currentContent={form.content || ""} onReplace={(newContent) => setForm({ ...form, previous_content: form.content, content: newContent })}/>)}
              </>}/>

          {/* 5. Opções avançadas — slug, SEO, vídeos, Instagram (recolhido por padrão) */}
          <EditorAdvancedSection values={{
            slug: form.slug ?? "",
            meta_title: form.meta_title ?? "",
            meta_description: form.meta_description ?? "",
            ...
            subtitle: form.subtitle ?? "",
            content: form.content ?? "",
        }} slugLocked={form.status === "publicada"} isNew={isNew} genIgHeadline={genIgHeadline} canGenIgHeadline={!!form.title} slugify={slugify} onChange={(patch) => setForm((f) => ({ ...f, ...patch }))} onGenerateIgHeadline={async () => {
            if (!form.title) {
                toast.error("Informe o título antes");
                return;
            }
            setGenIgHeadline(true);
            try {
                const { data, error } = await supabase.functions.invoke("generate-instagram-headline", { body: { title: form.title, subtitle: form.subtitle, excerpt: form.excerpt } });
                if (error)
                    throw error;
                if (!data?.headline)
                    throw new Error("Sem manchete");
                setForm((f) => ({ ...f, instagram_headline: data.headline }));
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 h-fit lg:max-h-[calc(100dvh-8rem)] lg:overflow-y-auto">
          <EditorPublicationPanel role={editorRole} status={currentStatus} isNew={isNew} saving={saving} saveState={saveState} lastServerSavedAt={lastServerSavedAt} isUrgent={!!form.is_urgent} urgentValidUntil={form.home_expires_at ?? ""} onToggleUrgent={(v) => setForm((f) => ({
            ...f,
            is_urgent: v,
            home_expires_at: v ? f.home_expires_at : "",
        }))} onUrgentValidUntilChange={(iso) => setForm((f) => ({ ...f, home_expires_at: iso }))} isDenuncia={!!form.is_denuncia} onToggleDenuncia={(v) => setForm((f) => ({ ...f, is_denuncia: v }))} scheduledAt={form.scheduled_at ?? ""} onCancelSchedule={canPublish && !isNew ? handleCancelSchedule : undefined} cancellingSchedule={cancellingSchedule} history={history} normalizeStatus={normalizeStatus} onAction={handleEditorAction} pinningSlot={<EditorPinningSection postId={isNew ? null : (id ?? null)} isPublished={form.status === "publicada"} canManage={isStaff} pinnedUntil={form.pinned_until ?? null} pinnedReason={form.pinned_reason ?? null} onChanged={async () => {
                if (isNew || !id)
                    return;
                ...
                if (data) {
                    setForm((f) => ({
                        ...
                        pinned_reason: data.pinned_reason,
                        pinned_slot: data.pinned_slot,
      ...
      {(() => {
            ...
            return (<EditorMobileActionBar primaryLabel={primary.label} onPrimary={() => handleEditorAction(primary.kind)} primaryDisabled={saving} onPreview={() => setPreviewOpen(true)} onSaveDraft={primary.kind === "save" ? undefined : () => handleEditorAction("save_draft")} canUnpublish={canPublish && currentStatus === "publicada"} onUnpublish={() => handleEditorAction("unpublish")}/>);
        })()}
