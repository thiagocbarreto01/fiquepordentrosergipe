...
interface Props {
  ...
  onApply: (result: { titulo: string; subtitulo: string; conteudo: string; resumo: string }) => void;
}

interface PreviewData {
  preview: { titulo: string; subtitulo: string; conteudo: string; resumo: string };
  stats: { original_chars: number; new_chars: number };
}

export default function CompletePostAIDialog({ postId, currentTitle, currentContent, onApply }: Props) {
          ...
          {!loading && data && (
            <div className="flex-1 overflow-auto space-y-4">
              ...
              <div>
                ...
                <div
                  className="border rounded p-3 text-sm max-h-[40vh] overflow-auto bg-background prose prose-sm max-w-none"
                  dangerouslySetInnerHTML={{ __html: data.preview.conteudo }}
                />
              </div>
