...
-- 1) Coluna dedicada para a imagem otimizada de compartilhamento social
ALTER TABLE public.posts
  ADD COLUMN IF NOT EXISTS share_image_url text,
  ADD COLUMN IF NOT EXISTS share_image_generated_at timestamptz;

ALTER TABLE public.posts_public
  ADD COLUMN IF NOT EXISTS share_image_url text,
  ADD COLUMN IF NOT EXISTS share_image_generated_at timestamptz;

-- 2) Invalidar share_image_url sempre que cover_image_url ou title mudar
CREATE OR REPLACE FUNCTION public.invalidate_share_image()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.cover_image_url IS DISTINCT FROM OLD.cover_image_url
      ...
      NEW.share_image_url := NULL;
      NEW.share_image_generated_at := NULL;
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_invalidate_share_image ON public.posts;
CREATE TRIGGER trg_invalidate_share_image
BEFORE UPDATE ON public.posts
FOR EACH ROW EXECUTE FUNCTION public.invalidate_share_image();

-- 3) Recriar sync_posts_public propagando os novos campos
  ...
  BEGIN
    IF NEW.status = 'publicada'::public.post_status
      ...
      INSERT INTO public.posts_public (
        ...
        share_image_url, share_image_generated_at
      ) VALUES (
        ...
        NEW.share_image_url, NEW.share_image_generated_at
      )
