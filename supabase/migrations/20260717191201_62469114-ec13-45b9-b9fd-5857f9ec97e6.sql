...
BEGIN
  ...
  IF _dry_run THEN
    RETURN jsonb_build_object(
      'success', true, 'dry_run', true, 'request_id', v_request_id,
      'planned_inserts', v_planned, 'existing_conflicts', v_existing,
      ...
      'rollback_snapshot', jsonb_build_object('tjse_before', v_source_update->'before')
    );
  END IF;
  ...
  IF (v_source_update->>'changes')::int > 0 THEN
    UPDATE public.news_sources
       SET url = v_tjse_new_url, source_type = v_tjse_new_type, updated_at = now()
     WHERE id = v_tjse_id;
    INSERT INTO public.source_allowed_hosts_batch_items(
      ...
      'source_updated', (v_source_update)::text
    );
    v_updated_sources_count := 1;
  END IF;

  UPDATE public.source_allowed_hosts_batches
     SET inserted_count = v_inserted_count, conflict_count = v_conflict_count
   WHERE id = v_batch_id;

  RETURN jsonb_build_object(
    'success', true, 'dry_run', false, 'request_id', v_request_id,
    'batch_id', v_batch_id, 'inserted_count', v_inserted_count,
    'conflict_count', v_conflict_count,
    'updated_sources_count', v_updated_sources_count, 'status', 'completed'
  );
END;
$$;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'success', true, 'dry_run', false, 'request_id', v_request_id,
    'batch_id', v_batch_id, 'removed_count', 1, 'status', 'completed'
  );
END;
$$;
