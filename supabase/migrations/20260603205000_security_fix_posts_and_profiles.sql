-- Security fixes:
-- 1) Remove direct public SELECT policy on posts table; make posts_public the
--    exclusive public interface by dropping security_invoker so the view runs
--    as its owner (bypassing RLS on the underlying table).
-- 2) Harden profiles_self_update so non-admins cannot modify role, status,
--    approved_by, or approved_at even if the column-level trigger is ever
--    bypassed or removed.

-- ============================================================
-- Fix A: posts_public view (remove security_invoker)
-- ============================================================
...
-- ============================================================
-- Fix B: drop the direct public policy on raw posts table
-- ============================================================
DROP POLICY IF EXISTS "published posts public" ON public.posts;

-- ============================================================
-- Fix C: harden profiles_self_update RLS policy
-- ============================================================
